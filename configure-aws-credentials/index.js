...
async function assumeRole(params) {
    ...
    assert(
        [sourceAccountId, roleToAssume, roleDurationSeconds, roleSessionName].every(isDefined),
...
function sanitizeGithubWorkflowName(name) {
    ...
    return nameWithoutSpecialCharacters.slice(0, MAX_TAG_VALUE_LENGTH)
}
...
async function outputAccountId(maskAccountId) {
...
function writeCredentialsFile(credentials, profile) {
    // Configure the AWS credentials with aws cli to the .credentials file.
    ...
    child_process.execFileSync('aws', ['configure', '--profile', profile, 'set', 'aws_access_key_id', accessKeyId], {stdio: 'inherit'});
    child_process.execFileSync('aws', ['configure', '--profile', profile, 'set', 'aws_secret_access_key', secretAccessKey], {stdio: 'inherit'});

    // Configure Session Token
    if (sessionToken) {
        child_process.execFileSync('aws', ['configure', '--profile', profile, 'set', 'aws_session_token', sessionToken], {stdio: 'inherit'});
    }
...
async function run() {
    try {
        ...
        if (accessKeyId) {
            ...
            // Write credentials to the shared credentials file
            // This is especially useful when assuming multiple roles and you don't want to loose
            // previous assumed credentials
            writeCredentialsFile({accessKeyId, secretAccessKey, sessionToken}, profile);
        }
        ...
        const sourceAccountId = await outputAccountId(maskAccountId);
        ...
        if (roleToAssume) {
            ...
            writeCredentialsFile(roleCredentials, profile);
            await validateCredentials(roleCredentials.accessKeyId);
            await outputAccountId(maskAccountId);
        }
    } catch (error) {
